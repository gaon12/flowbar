...
import { displayWidth, formatAmount, formatBytes, formatDuration, formatRate, padLeft, pluralizeUnit, stringifyPostfix, truncateDisplay, } from "./display.js";
...
export function buildDeterminateLine(snapshot, width) {
...
export function buildCountingLine(snapshot, width) {
    const { options } = snapshot;
    const label = options.label ? `${options.label}  ` : "";
...
export function buildIndeterminateLine(snapshot, width) {
    const { options } = snapshot;
    const label = options.label ? `${options.label}  ` : "";
    const elapsed = formatDuration(snapshot.timing.elapsedMs);
